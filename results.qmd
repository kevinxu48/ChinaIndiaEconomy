...
## Stacked bar chart - Energy usage China/India
```{r,fig.height=20,fig.width=30}
colors = c("darkred","yellow3","tan","green3")

ggplot(energyrecent, aes(x = `Country`, y= Percentages, fill = factor(`Energy Usage Proportions`, levels = c("coal","natural gas","petroleum","clean")))) +
...
```
Next, we will explore why China and India are considered the two of the most polluted countries in the world and are thus at high risk of environmental issues like climate change and resource depletion. From the plot we see that since the year 2010, only a small fraction of both China and India's energy production comes from clean energy sources such as nuclear energy and renewable sources like solar and wind energy. Moreover, both countries rely on coal as their primary energy source which according to the EPA and environmental organizations is the biggest emitter of greenhouse gasses along with natural gasses. China has higher levels of coal usage and lower amounts of renewable energy, but there is a promising increase renewable energy sources in recent years. As of 2022, almost 20% of both countries' energy production are from clean energy sources such as nuclear or renewable energy.
...
## Line plot - Emissions and Gain score over time for China/India  
```{r}
...
## Heat map - Gain index for G20 countries 
```{r}
...
## Line plot - Population and labor force China/India
```{r}
pop <- read.csv("Population.csv", header = TRUE, check.names = FALSE)
LF <- read.csv("LaborForce.csv", header = TRUE, check.names = FALSE)

pop <- pivot_longer(pop, cols='1960':'2022', names_to = "year", values_to = "population")
...
#popLF <- merge(pop, LF, by = c("year", "Country"))

pop_plot <- pop %>% filter(Country %in% c("India", "China")) %>% ggplot() +
  geom_line(aes(x = year, y = population, color = Country), linewidth = 1) +
  scale_color_manual(values = custom_colors) +
  #geom_point(data = data.frame(year = 2021, population = 1400000000), aes(year, population),
            # color = "black", size = 11, alpha = 0.3, shape = 1) +
  labs(x = "", y = "", title = "Total population") +
...
LF_plot <- LF %>% filter(Country %in% c("India", "China") & year >= 1990) %>% ggplot() +
  ...
  #geom_point(data = data.frame(year = 2021, population = 1400000000), aes(year, population),
  # color = "black", size = 11, alpha = 0.3, shape = 1) +
  labs(x = "", y = "", title = "Labor force") +
  scale_y_continuous(labels = scales::label_comma()) +
  kv_theme()

grid.arrange(pop_plot, LF_plot, ncol = 2, top = "Population versus labor force over time")
